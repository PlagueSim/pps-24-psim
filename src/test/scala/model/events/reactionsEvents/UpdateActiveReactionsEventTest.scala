...
class UpdateActiveReactionsEventTest extends AnyFlatSpec with Matchers:
  ...
  trait ConditionFactory {
    def apply(result: Boolean): ReactionCondition = new ReactionCondition {
      override def isSatisfied(state: SimulationState, nodeId: String): Boolean = result
    }
  }
  object ConditionFactory extends ConditionFactory

  def ruleWithCondition(result: Boolean) =
    ReactionRule(ConditionFactory(result), nodeId => ReactionAction.CloseEdges(EdgeType.Land, nodeId))
  ...
  def simulationState(
      ...
      infectionLogic: model.infection.InfectionAndDeathPopulation.PopulationStrategy = null,
      deathLogic: model.infection.InfectionAndDeathPopulation.PopulationStrategy = null,
  ...
  "UpdateActiveReactionsEvent" should "remove expired reactions and activate new ones" in:
    ...
    val reactions = Reactions(rules = List(rule), activeReactions = Set(expired))
