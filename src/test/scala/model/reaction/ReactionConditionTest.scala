...
class ReactionConditionTest extends AnyFlatSpec with Matchers:
  def testSimulationState: SimulationState =
    ...
    val initialNodes = Map("A" -> defaultNode, "B" -> defaultNode, "C" -> defaultNode)
    val initialWorld = World(initialNodes, Set.empty, Map(model.world.Static -> 1.0))
    SimulationState(BasicYear(Day(0), Year(2023)), Disease("TestDisease", Set.empty, 1), Cure(), initialWorld)

  def simulationStateWithInfected(nodeId: String, infectedCount: Int): SimulationState =
    val baseState = testSimulationState
    val node = baseState.world.nodes(nodeId)
    val updatedNode = model.world.Node.Builder(node.population, infectedCount, node.cureEffectiveness).build()
    val updatedWorld = World(baseState.world.nodes.updated(nodeId, updatedNode), baseState.world.edges, baseState.world.movements)
