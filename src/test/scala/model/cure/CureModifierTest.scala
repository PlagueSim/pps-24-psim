...
class CureModifierTest extends AnyFlatSpec with Matchers:
  val nodeId = ModifierId(ModifierSource.Node(NodeId("testNode")), ModifierKind.Multiplier)
  val mutationId = ModifierId(ModifierSource.Mutation(MutationId("testMutation")), ModifierKind.Additive)
  ...
  "ModifierId" should "implement proper equality" in:
    val id1 = ModifierId(ModifierSource.Node(NodeId("A")), ModifierKind.Additive)
    val id2 = ModifierId(ModifierSource.Node(NodeId("A")), ModifierKind.Additive)
    val id3 = ModifierId(ModifierSource.Node(NodeId("B")), ModifierKind.Additive)
