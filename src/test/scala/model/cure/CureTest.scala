...
class CureTest extends AnyFlatSpec with Matchers:
  ...
  it should "calculate effective speed with multiplier modifier" in:
    val modifiers = CureModifiers.empty.add(CureModifier.Multiplier(2.0))
    val cure = defaultCure(baseSpeed = 0.1, modifiers = modifiers)
    cure.effectiveSpeed shouldBe (0.2 +- 1e-9)

  it should "calculate effective speed with multiple modifiers" in:
    val modifiers = CureModifiers.empty.add(CureModifier.Multiplier(2.0)).add(CureModifier.Additive(0.1))
    val cure = defaultCure(baseSpeed = 0.1, modifiers = modifiers)
    cure.effectiveSpeed shouldBe (0.3 +- 1e-9)
