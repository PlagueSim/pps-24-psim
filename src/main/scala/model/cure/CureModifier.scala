 ...
 *
 * @note Modifiers can be either persistent (applied daily) or one-time (applied immediately)
 */
...
object CureModifier:
   ...
   * @param factor Multiplication factor (must be >= 0.0)
   * @return Some(Multiplier) if valid, None otherwise
   */
   ...
   *
   * @param id Unique identifier for the modifier
   * @param amount Value to add to cure speed (must be in [-1.0, 1.0])
   * @return Some(Additive) if valid, None otherwise
   */
   ...
   *
   * @param id Unique identifier for the modifier
   * @param amount Value to add to cure progress (must be in [-1.0, 1.0])
   * @return Some(ProgressModifier) if valid, None otherwise
   */
  def progressModifier(id: ModifierId, amount: Double): Option[ProgressModifier] =
