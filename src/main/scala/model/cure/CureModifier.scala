...
object CureModifier:
  ...
  def progressModifier(id: ModifierId, amount: Double): Option[ProgressModifier] =
    if amount >= -1.0 && amount <= 1.0 then Some(ProgressModifier(id, amount)) else None
  ...
  /** Multiplies the base speed by a factor.
    ...
    */
  private[CureModifier] case class Multiplier (id: ModifierId, factor: Double) extends PersistentModifier:
  ...
  /** Adds a fixed amount to the base speed.
    ...
    */
  private[CureModifier] case class Additive(id: ModifierId, amount: Double) extends PersistentModifier:
   ...
   */
  private[CureModifier] case class ProgressModifier(id: ModifierId, amount: Double) extends OneTimeModifier:
